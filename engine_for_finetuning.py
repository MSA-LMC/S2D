...
def train_class_batch(model, samples, a, target, criterion):
    outputs, feat = model(samples, a)
...
def train_one_epoch(model: torch.nn.Module, model2, sdl,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            with torch.cuda.amp.autocast():
                ...
                loss, output, feat = train_class_batch(
                    model, samples, a, targets, criterion)
        ...
        if sdl_loss_fn and epoch%sdl_update_freq==0 and epoch >20:
            with torch.cuda.amp.autocast():
                with torch.no_grad():
                    a = model2(ori_samples)
                    outputs, feat = model(ori_samples, a)
...
@torch.no_grad()
def validation_one_epoch(data_loader, model, model2, device):
...
@torch.no_grad()
def final_test(data_loader, model, model2, device, file, save_feature=False):
    criterion = torch.nn.CrossEntropyLoss()
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            # else:
            a = model2(videos)
            output, saved_feature = model(videos, a)
            loss = criterion(output, target)
